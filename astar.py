    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                val = grid[row][col]
                if not start and val != end: #Makes sure that start and end nodes are not the same node
                    start = val
                    start.make_start()

                elif not end and val != start:
                    end = val
                    end.make_end()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    PathFinder(lambda: draw_board(win, grid, ROWS, width), grid, start, end)
